<template>
  <div class="space-y-8 flex flex-col items-center min-w-max">
    <div 
      ...
      :key="pageIndex" 
      class="a4-page text-gray-900 dark:text-gray-100 w-[794px] min-h-[1123px] mx-auto bg-white dark:bg-[#111827] shadow-[0_10px_40px_rgba(0,0,0,0.1)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.5)] ring-1 ring-gray-200 dark:ring-white/10 rounded-sm p-12 shrink-0 relative"
      style="page-break-after: always; break-after: page;"
    >
      <!-- Header Report -->
      <div v-if="page.showHeader" class="flex flex-row items-center justify-between gap-6 mb-6 border-b-2 border-gray-900/10 dark:border-white/10 pb-4">
        <!-- Logo Kiri -->
        ...
        <div class="text-right space-y-1 flex-1 flex flex-col justify-center mt-0">
          <h1 class="text-lg font-bold text-gray-900 dark:text-white uppercase tracking-wide leading-snug">
      ...
      <div v-if="page.pemasukan.length > 0 || page.showPemasukanHeader" class="space-y-2 mb-5">
        <div v-if="page.showPemasukanHeader" class="flex items-center gap-2 pb-1.5 border-b border-gray-300 dark:border-white/10">
          <span class="w-5 h-5 rounded bg-emerald-100 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 font-bold text-[10px] flex items-center justify-center">A</span>
          <h3 class="font-bold text-gray-900 dark:text-white uppercase text-xs tracking-wider">Pemasukan <span v-if="!page.showHeader" class="text-[10px] text-gray-400 dark:text-gray-500 font-normal ml-2 italic">(Lanjutan)</span></h3>
        </div>
          ...
          <table class="w-full text-xs text-left border-collapse table-fixed">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-white/5">
              <tr v-for="(item, index) in page.pemasukan" :key="index" class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                <td class="py-1.5 px-3 text-center text-gray-500 dark:text-gray-400">{{ item.no }}</td>
                <td class="py-1.5 px-3 text-center text-gray-500 dark:text-gray-400">{{ item.tanggal || '-' }}</td>
                <td class="py-1.5 px-3 text-gray-700 dark:text-gray-300 pr-4">
                  <div class="font-normal leading-snug truncate">{{ item.uraian }}</div>
                </td>
                <td class="py-1.5 px-3 text-right font-medium text-gray-900 dark:text-white">{{ formatRupiah(item.jumlah) }}</td>
              </tr>
            </tbody>
            <tfoot v-if="page.showPemasukanTotal">
              <tr class="bg-emerald-50 dark:bg-emerald-950/20 border-y-2 border-emerald-200 dark:border-emerald-900/30">
                <td colspan="3" class="py-1.5 px-2 font-bold text-gray-900 dark:text-white uppercase text-right">JUMLAH PEMASUKAN</td>
                <td class="py-1.5 px-2 text-right font-bold text-emerald-600 dark:text-emerald-400">{{ formatRupiah(report.totalPemasukan) }}</td>
              </tr>
            </tfoot>
          </table>
        </div>
      ...
      <div v-if="page.pengeluaran.length > 0 || page.showPengeluaranHeader" class="space-y-2 mb-5">
        <div v-if="page.showPengeluaranHeader" class="flex items-center gap-2 pb-1.5 border-b border-gray-300 dark:border-white/10">
          <span class="w-5 h-5 rounded bg-rose-100 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 font-bold text-[10px] flex items-center justify-center">B</span>
          <h3 class="font-bold text-gray-900 dark:text-white uppercase text-xs tracking-wider">Pengeluaran <span v-if="!page.showHeader && page.pemasukan.length === 0" class="text-[10px] text-gray-400 dark:text-gray-500 font-normal ml-2 italic">(Lanjutan)</span></h3>
        </div>
          ...
          <table class="w-full text-xs text-left border-collapse table-fixed">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-white/5">
              <tr v-for="(item, index) in page.pengeluaran" :key="index" class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                ...
                <td class="py-1.5 px-3 text-right font-medium text-gray-900 dark:text-white">{{ formatRupiah(item.shadow || item.jumlah) }}</td>
              </tr>
            </tbody>
            <tfoot v-if="page.showPengeluaranTotal">
              <tr class="bg-rose-50 dark:bg-rose-950/20 border-y-2 border-rose-200 dark:border-rose-900/30">
                <td colspan="3" class="py-1.5 px-2 font-bold text-gray-900 dark:text-white uppercase text-right">JUMLAH PENGELUARAN</td>
                <td class="py-1.5 px-2 text-right font-bold text-rose-600 dark:text-rose-400">{{ formatRupiah(report.totalPengeluaran) }}</td>
              </tr>
            </tfoot>
          </table>
        </div>
      ...
      <div v-if="page.showSaldo" class="mt-6 pt-2 break-inside-avoid mb-6">
        <div class="flex items-center justify-between border-y-2 border-cyan-500 dark:border-cyan-900/30 bg-cyan-50 dark:bg-cyan-950/20 px-4 py-2.5">
          <div class="flex items-center gap-3">
            <span class="w-6 h-6 rounded-full bg-cyan-100 dark:bg-cyan-950/30 text-cyan-600 dark:text-cyan-400 font-bold text-xs flex items-center justify-center shrink-0">C</span>
            <div>
              <p class="font-bold text-gray-900 dark:text-white uppercase text-xs">Sisa Saldo (A - B)</p>
              <p class="text-[10px] italic text-gray-500 dark:text-gray-400 line-clamp-1 mt-0.5">"{{ report.terbilang }}"</p>
            </div>
          </div>
          <div class="text-xl font-bold font-heading text-cyan-700 dark:text-cyan-400 shrink-0">
            Rp {{ formatRupiah(report.sisaSaldo) }}
          </div>
        </div>
        <div v-if="report.keterangan" class="mt-2 px-1">
          <p class="text-[10px] text-gray-500 dark:text-gray-400"><span class="font-bold">Catatan:</span> {{ report.keterangan }}</p>
        </div>
      </div>
      ...
      <!-- Page Number -->
      <div class="absolute bottom-6 right-10 text-xs text-gray-400 dark:text-gray-500 font-medium print:hidden">
