<template>
  <div>
            ...
            <tr v-for="(item, index) in adminStore.kegiatan" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-white/[0.03] transition-colors group">
              ...
              <td class="py-4 px-6">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded-xl bg-gray-100 dark:bg-white/5 flex items-center justify-center shrink-0 border border-gray-200 dark:border-white/10 overflow-hidden text-center">
                    <div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-[60] flex items-center justify-center px-4">
      ...
      <div class="bg-white dark:bg-[#112323] border border-gray-200 dark:border-white/10 rounded-3xl w-full max-w-xl relative z-10 shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-modal-enter">
          ...
          <form @submit.prevent="saveKegiatan" class="space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                >
                  <option class="bg-white dark:bg-slate-800" value="Kajian">Kajian</option>
                  ...
                  <option class="bg-white dark:bg-slate-800" value="Umum">Umum</option>
                </select>
              </div>
            </div>
        ...
        <div class="p-6 border-t border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 flex items-center justify-end gap-3 shrink-0">
          ...
          <button 
            @click="saveKegiatan"
            class="px-6 py-2.5 rounded-xl bg-secondary hover:bg-secondary-light text-white dark:text-dark font-bold shadow-lg shadow-secondary/20 transition-all active:scale-95"
          >
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-[70] flex items-center justify-center px-4">
      ...
      <div class="bg-white dark:bg-[#112323] border border-gray-200 dark:border-white/10 rounded-3xl w-full max-w-sm relative z-10 shadow-2xl p-6 text-center animate-modal-enter">
        ...
        <p class="text-gray-500 dark:text-white/60 text-sm mb-8">
          Tindakan ini tidak dapat dibatalkan. Kegiatan akan dihapus dari sistem secara permanen.
        </p>
        <div class="flex items-center gap-3 w-full">
          <button 
            @click="closeDeleteModal"
            class="flex-1 py-3 rounded-xl bg-gray-100 dark:bg-white/5 text-gray-700 dark:text-white/70 font-semibold hover:bg-gray-200 dark:hover:bg-white/10 transition-all active:scale-95"
          >
            Batal
          </button>
          <button 
...
<script setup>
import { ref } from 'vue'
import { Plus, Edit, Trash2, X, Calendar, Clock, MapPin, User, AlertCircle } from 'lucide-vue-next'
...
const getDefaultForm = () => ({
  ...
  location: '',
  badge: '',
  image: null // Admin dashboard uses default from store later
})
...
const saveKegiatan = () => {
  ...
  if (isEditing.value) {
    ...
    if (index !== -1) {
      ...
      localStorage.setItem('admin_kegiatan', JSON.stringify(adminStore.kegiatan))
    }
    ...
    // Pick a default image if available
    let defImg = null
    if (adminStore.kegiatan.length > 0) {
